...
class BYTETracker {
 public:
    ...
    BYTETracker(const int& frame_rate, const int& track_buffer, const float& track_thresh = 0.5f,
                const float& high_thresh = 0.6f, const float& match_thresh = 0.8f);
    ...
    std::vector<STrackPtr> update(const std::vector<Object>& objects, const std::vector<BlobObject>& blob_objects,
                                  const EgoMotionData& ego_motion, int64_t timestamp_ns = 0);
    ...
    std::vector<STrackPtr> update(const std::vector<Object>& objects, int64_t timestamp_ns = 0);
 ...
 private:
    ...
    void applyEgoMotionCorrection(std::vector<STrackPtr>& tracks, const EgoMotionData& ego_motion);
